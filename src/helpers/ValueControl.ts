import { concat, EMPTY, merge, Observable, of, ReplaySubject, SubjectLike } from "rxjs"
import { map, mergeMapTo, pluck, shareReplay, switchMap, tap } from "rxjs/operators"
...
export class ValueControl<T> {
    ...
    constructor(payload: ValueControlPayload<T>) {
        ...
        const set$ = merge(this.setSubject as unknown as Observable<T>, initial$)
            .pipe(
                tap((v) => debug('set', v)),
                switchMap(value => {
                    debug('publishing!')
        ...
        this.state$ = merge(this.payload.run$.pipe(mergeMapTo(EMPTY)), set$)
            .pipe(shareReplay(1))
    }
