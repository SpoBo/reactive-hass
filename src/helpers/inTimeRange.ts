...
function isAfter(
  timestamp: HoursAndMinutes,
  reference: HoursAndMinutes
): boolean {
  return (
    (timestamp.hours === reference.hours &&
      timestamp.minutes >= reference.minutes) ||
    timestamp.hours > reference.hours
  );
}
...
export default function inTimeRange(start: string, end: string) {
  const starting = parseTimeString(start);
  const ending = parseTimeString(end);
