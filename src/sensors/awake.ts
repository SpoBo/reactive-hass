...
export default function awake$(
  ...
  const hadMotionToday$ = history.entity$(DOWNSTAIRS_MOTION_SENSORS[0], startOfDay(new Date())).pipe(
    map((v) => v.some(v => v.state === 'on'))
  );

  // Real-time motion detection
  const currentMotion$ = states.entity$(DOWNSTAIRS_MOTION_SENSORS[0]).pipe(
    map((v) => v.state === 'on')
  );
